...
#         # Annotate slope
#         #plt.text(df_temp["Date"].iloc[-1], theil_pred[-1], f"{theil_slope:.3f} °C/yr", fontsize=9, color=trend_colors[temp_type], fontweight='bold')
#         plt.text(df_temp["Date"].iloc[-1] - pd.Timedelta(days=500),  # shift a bit left
#          theil_pred[-1], 
...
#             if valid_years_with_data < 5: # ensures only decades with at least 4 years of valid data are included in the results
...
#     extra_df = pd.read_excel(file_path)

#     if extra_df.empty:
#         return merged_df

#     # Extract and reformat date columns from Excel format
...
#     # Ensure merged_df also has datetime
...
#         match = merged_df['Date'] == date
#         if match.any():
#             for var in variables_to_update:
#                 if pd.isna(merged_df.loc[match, var].values[0]) and not pd.isna(row[var]):
#                     merged_df.loc[match, var] = row[var]
#                     merged_df.loc[match, 'Source'] = 'Extra'
#         else:
...
def data_formatting(input_folder_path, output_folder_path, metadata_file_path, formatted_already_digitized_data_dir, station, date_column,
    ...
    #     valid_years = valid_years[valid_years > 30].index  # Only keep years with >30 days of data
    ...
    #     else:
    #         norm = plt.Normalize(min(years), max(years))  # Normalize over valid year range
    #         cmap_func = plt.cm.get_cmap('Blues')
    #         year_to_color = {year: cmap_func(norm(year)) for year in years}

    #         fig, ax = plt.subplots(figsize=(14, 6))
    #         for year in years:
    #             yearly_data = df_anomaly[df_anomaly['Year'] == year]
    #             ax.plot(yearly_data['DayOfYear'], yearly_data['Anomaly'], color=year_to_color[year], alpha=0.9)
    #             last_day = yearly_data['DayOfYear'].max()
    #             last_anomaly = yearly_data[yearly_data['DayOfYear'] == last_day]['Anomaly'].values[0]
    #             ax.text(last_day + 1, last_anomaly, str(year), color=year_to_color[year], fontsize=8, va='center')
    ...
    # # === Plot 4: Yearly Temperature Trends with 14-day Rolling Average ===
    # for column in ["Max_Temperature", "Min_Temperature", "Avg_Temperature"]:
    #     cmap = {'Max_Temperature': 'Reds', 'Min_Temperature': 'Blues', 'Avg_Temperature': 'Oranges'}[column]
    #     # Convert the column to numeric (coerce string "NaN" or other junk to actual np.nan)
    #     merged_df[column] = pd.to_numeric(merged_df[column], errors='coerce')

    #     # Now filter only years with actual data
    ...
    #     norm = plt.Normalize(min(valid_years), max(valid_years))  # Normalize over valid years
    #     cmap_func = plt.cm.get_cmap(cmap)
    #     year_to_color = {year: cmap_func(norm(year)) for year in valid_years}

    #     fig, ax = plt.subplots(figsize=(12, 6))
    #     for year in valid_years:
    #         df_year = merged_df[merged_df['Year'] == year].copy()
    #         df_year[f'{column}_5day'] = df_year[column].rolling(window=14, min_periods=1).mean()
    #         ax.plot(df_year['Date'].dt.dayofyear, df_year[f'{column}_5day'], label=str(year), color=year_to_color[year])
    #         ax.text(df_year['Date'].dt.dayofyear.iloc[-1], df_year[f'{column}_5day'].iloc[-1], str(year),
    #                 fontsize=8, ha='left', color=year_to_color[year])
...
#     """
#     Plot interpolated warming trends (Tx, Tn, Tavg) using Cartopy with DRC shapefile.

#     Parameters
#     ----------
#     trend_df : pd.DataFrame
#         DataFrame with columns: ['station_id', 'station_name', 'lat', 'lon', 'trend_tx', 'trend_tn', 'trend_tavg']
#     drc_shapefile_path : str
...
#     variables = {
#         'trend_tx': 'Max Temperature Trend (°C/yr)',
#         'trend_tn': 'Min Temperature Trend (°C/yr)',
#         'trend_tavg': 'Avg Temperature Trend (°C/yr)'
#     }
...
#         # Add DRC shapefile
#         region_shape = ShapelyFeature(Reader(region_shapefile_path).geometries(),
#                                    ccrs.PlateCarree(), edgecolor='black', facecolor='none', linewidth=1.2)
#         ax.add_feature(region_shape)

#         # Extract valid data
#         subset = trend_df.dropna(subset=[var_key])
...
#         ax.scatter(lons, lats, color='black', edgecolors='white', s=40, label='Stations')
